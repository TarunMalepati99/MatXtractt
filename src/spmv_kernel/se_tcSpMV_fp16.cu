                    ...
                    half *X_val, half *Y_val, int *order_rid, int rowA, int colA, indT nnzA, int NUM, double threshold, int block_longest)
{
    ...
    indT *rptA = (indT *)malloc(sizeof(indT) * (row_block + 1));
    memset(rptA, 0, sizeof(indT) * (row_block + 1));
    indT *long_rpt = (indT *)malloc(sizeof(indT) * (row_long + 1));
    memset(long_rpt, 0, sizeof(indT) * (row_long + 1));
