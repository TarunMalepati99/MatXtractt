...
__device__ __forceinline__ double warpReduceSum_all_fp64(double sum)
{
...
__global__ void longPart_sum(int *dlongA_rpt, double *dwarp_val, double *dY_val, int row_long)
{
    int bid = blockIdx.x;
    int tid = threadIdx.x;
    int laneid = 31 & tid;
    ...
    double *cur_temp_val = dwarp_val + offset_longA;
    ...
    thread_val = warpReduceSum_all_fp64(thread_val);
...
__global__ void dasp_spmv2(double *dX_val, double *dY_val,
                          double *dlongA_val, int *dlongA_cid, double *dwarp_val, int *dlongA_rpt, int row_long,
                          double *dregA_val, int *dregA_cid, int *dblockA_ptr, int row_block, int blocknum, 
                          double *dirregA_val, int *dirregA_cid, int *dirregA_rpt,
                          double *dshort_val, int *dshort_cid, int short_row_1, int common_13, int short_row_34, int short_row_2,
                          int offset_reg, int offset_short1, int offset_short13, int offset_short34, int offset_short22,
                          int fill0_nnz_short13, int fill0_nnz_short34)
{
    int bid = blockIdx.x;
    int tid = threadIdx.x;
    int laneid = 31 & tid;
    ...
    if (bid < offset_reg)
    {
        ...
        double *curA_val = dlongA_val + offset;
        ...
        // // double *cur_temp_val = dwarp_val + dlongA_rpt[global_warpid];
        // int offset_longA = load_int_from_global(dlongA_rpt + global_warpid);
        // double *cur_temp_val = dwarp_val + offset_longA;
        // int len = load_int_from_global(dlongA_rpt + global_warpid + 1) - offset_longA;
    ...
    else if (bid >= offset_reg && bid < offset_short1)
    {
        ...
        if (rowloop == 1)
        {
            ...
            double *curA_val = dregA_val + offset_A;
    ...
    else if (bid >= offset_short1 && bid < offset_short13)
    {
        ...
        double temp_y = load_double_from_global(dshort_val + tid1) * dX_val[x_idx];
    ...
    else if (bid >= offset_short13 && bid < offset_short34)
    {
        ...
        double fragA = 0.0, fragB = 0.0, fragC[2], res;
        int groupID = laneid >> 2;
        int tID_in_group = 3 & laneid;
        ...
        for (int i = 0; i < groupNum; i ++)
        {
            ...
            double *cur_val = dshort_val + offset;
...
// In order version
void se_tcspmv_fp64(double *csrValA, indT *csrRowPtrA, int *csrColIdxA,
                    double *X_val, double *Y_val, int *order_rid, int rowA, int colA, indT nnzA, int NUM, double threshold, int block_longest)
{
    printf("\n dasp!!!!!!!!!!! \n");
    ...
    [[maybe_unused]] int nnz_short, nnz_long, origin_nnz_reg, fill0_nnz_reg, nnz_irreg;
    ...
    int fill0_nnz_short = short_row_1 + fill0_nnz_short13 + fill0_nnz_short34 + fill0_nnz_short22;
    double *short_val = (double *)malloc(sizeof(double) * fill0_nnz_short);
    int *short_cid = (int *)malloc(sizeof(int) * fill0_nnz_short);
    memset(short_val, 0.0, sizeof(double) * fill0_nnz_short);
    memset(short_cid, 0, sizeof(int) * fill0_nnz_short);

    int super_group = 1 + threadblock13 + threadblock34 + threadblock22;
    int *superX_ptr = (int *)malloc(sizeof(int) * (super_group + 1));
    ...
    {
        double *cur_short_val = short_val + short_row_1 + i * MMA_M * MMA_K;
    ...
    {
        // double *cur_short_val = short_val + short_row_1 + short_block13 * MMA_M * MMA_K + i * MMA_K;
        // int *cur_short_cid = short_cid + short_row_1 + short_block13 * MMA_M * MMA_K + i * MMA_K;
        double *cur_short_val = short_val + short_row_1 + fill0_nnz_short13 + i * MMA_K;
    ...
    {
        double *cur_short_val = short_val + short_row_1 + fill0_nnz_short13 + (short_row_3 + i) * MMA_K;
    ...
    {
        double *cur_short_val = short_val + short_row_1 + fill0_nnz_short13 + fill0_nnz_short34 + i * MMA_M * MMA_K;
    ...
    int *cur_superX_ptr = superX_ptr + 1;
    ...
    int nnz_superX = superX_ptr[super_group];
    ...
    double *superX_val = (double *)malloc(sizeof(double) * nnz_superX);
    ...
    double *short3_val = (double *)malloc(sizeof(double) * nnz_short);
    int *short3_cid = (int *)malloc(sizeof(int) * nnz_short);
    int *short3_rpt = (int *)malloc(sizeof(int) * (short_row + 1));
    ...
    {
        int idx = order_rid[offset_short_row + i];
        memcpy(short3_val + short3_rpt[i], csrValA + csrRowPtrA[idx], sizeof(double) * (csrRowPtrA[idx + 1] - csrRowPtrA[idx]));
    ...
    int *long_rpt_new = (int *)malloc(sizeof(int) * (row_long + 1));
    memset(long_rpt_new, 0, sizeof(int) * (row_long + 1));
    int warp_number = 0;
    ...
    double *val_by_warp = (double *)malloc(sizeof(double) * warp_number);
    int *rid_by_warp = (int *)malloc(sizeof(int) * warp_number);
    double *long_val = (double *)malloc(sizeof(double) * fill0_nnz_long);
    memset(long_val, 0.0, sizeof(double) * fill0_nnz_long);
    ...
    for (int i = 0; i < row_long; i ++)
    {
        double *cur_val = long_val + long_rpt_new[i] * loopNum_long * MMA_M * MMA_K;
    ...
    int *blockPtr = (int *)malloc(sizeof(int) * (blocknum + 1));
    memset(blockPtr, 0, sizeof(int) * (blocknum + 1));

    int *irreg_rpt = (int *)malloc(sizeof(int) * (row_block + 1));
    memset(irreg_rpt, 0, sizeof(int) * (row_block + 1));
    ...
    double *irreg_val = (double *)malloc(sizeof(double) * nnz_irreg);
    ...
    double *reg_val = (double *)malloc(sizeof(double) * fill0_nnz_reg);
    ...
    for (int bid = 0; bid < blocknum; bid ++)
    {
        ...
        double *temp_val = (double *)malloc(sizeof(double) * nnz_block);
        int *temp_cid = (int *)malloc(sizeof(int) * nnz_block);
        double *cur_val = reg_val + blockPtr[bid];
        ...
        memcpy(cur_val, temp_val, sizeof(double) * nnz_block);
    ...
    long long int data_X = (rowA + colA) * sizeof(double) + \
                           fill0_nnz_long * (sizeof(double) + sizeof(int)) + warp_number * sizeof(double) + (row_long + 1) * sizeof(int) + \
                           fill0_nnz_short * (sizeof(double) + sizeof(int)) + \
                           fill0_nnz_reg * (sizeof(double) + sizeof(int)) + (blocknum + 1) * sizeof(int) + \
                           nnz_irreg * (sizeof(double) + sizeof(int)) + (row_block + 1) * sizeof(int);
    
    long long int data_X2 = (rowA + nnzA) * sizeof(double) + \
                            fill0_nnz_long * (sizeof(double) + sizeof(int)) + warp_number * sizeof(double) + (row_long + 1) * sizeof(int) + \
                            fill0_nnz_short * (sizeof(double) + sizeof(int)) + \
                            fill0_nnz_reg * (sizeof(double) + sizeof(int)) + (blocknum + 1) * sizeof(int) + \
                            nnz_irreg * (sizeof(double) + sizeof(int)) + (row_block + 1) * sizeof(int);
    ...
    cudaMalloc((void **)&dshort3_val, sizeof(double) * nnz_short);
    cudaMalloc((void **)&dshort3_cid, sizeof(int) * nnz_short);
    cudaMalloc((void **)&dshort3_rpt, sizeof(double) * (short_row + 1));
    cudaMemcpy(dshort3_cid, short3_cid, sizeof(int) * nnz_short, cudaMemcpyHostToDevice);
    cudaMemcpy(dshort3_val, short3_val, sizeof(double) * nnz_short, cudaMemcpyHostToDevice);
    cudaMemcpy(dshort3_rpt, short3_rpt, sizeof(int) * (short_row + 1), cudaMemcpyHostToDevice);
    ...
    double *dreg_val, *dirreg_val;
    int *dblock_ptr, *dirreg_rpt;
    int *dreg_cid, *dirreg_cid;

    cudaMalloc((void **)&dX_val, sizeof(double) * colA);
    cudaMalloc((void **)&dY_val, sizeof(double) * rowA);
    cudaMemcpy(dX_val, X_val, sizeof(double) * colA, cudaMemcpyHostToDevice);
    cudaMemset(dY_val, 0.0, sizeof(double) * rowA);

    cudaMalloc((void **)&dlong_val, sizeof(double) * fill0_nnz_long); 
    cudaMalloc((void **)&dlong_cid, sizeof(int) * fill0_nnz_long);
    cudaMalloc((void **)&drid_by_warp, sizeof(int) * warp_number);
    cudaMalloc((void **)&dval_by_warp, sizeof(double) * warp_number);
    cudaMalloc((void **)&dlong_ptr_warp, sizeof(int) * (row_long + 1));
    cudaMemcpy(dlong_val, long_val, sizeof(double) * fill0_nnz_long, cudaMemcpyHostToDevice);
    ...
    cudaMemcpy(dlong_ptr_warp, long_rpt_new, sizeof(int) * (row_long + 1), cudaMemcpyHostToDevice);

    cudaMalloc((void **)&dsuperX_ptr, sizeof(int) * (super_group + 1));
    cudaMalloc((void **)&dsuperX_val, sizeof(double) * nnz_superX);
    cudaMalloc((void **)&dshort_cid_new, sizeof(int) * new_cid_len);
    cudaMemcpy(dsuperX_ptr, superX_ptr, sizeof(int) * (super_group + 1), cudaMemcpyHostToDevice);
    cudaMemcpy(dsuperX_val, superX_val, sizeof(double) * nnz_superX, cudaMemcpyHostToDevice);
    cudaMemcpy(dshort_cid_new, short_cid_new, sizeof(int) * new_cid_len, cudaMemcpyHostToDevice);

    cudaMalloc((void **)&dshort_val, sizeof(double) * fill0_nnz_short);
    cudaMalloc((void **)&dshort_cid, sizeof(int) * fill0_nnz_short);
    cudaMemcpy(dshort_val, short_val, sizeof(double) * fill0_nnz_short, cudaMemcpyHostToDevice);
    cudaMemcpy(dshort_cid, short_cid, sizeof(int) * fill0_nnz_short, cudaMemcpyHostToDevice);

    cudaMalloc((void **)&dreg_val, sizeof(double) * fill0_nnz_reg);
    cudaMalloc((void **)&dreg_cid, sizeof(int) * fill0_nnz_reg);
    cudaMalloc((void **)&dblock_ptr, sizeof(int) * (blocknum + 1));
    cudaMemcpy(dreg_val, reg_val, sizeof(double) * fill0_nnz_reg, cudaMemcpyHostToDevice);
    cudaMemcpy(dreg_cid, reg_cid, sizeof(int) * fill0_nnz_reg, cudaMemcpyHostToDevice);
    cudaMemcpy(dblock_ptr, blockPtr, sizeof(int) * (blocknum + 1), cudaMemcpyHostToDevice);

    cudaMalloc((void **)&dirreg_val, sizeof(double) * nnz_irreg);
    cudaMalloc((void **)&dirreg_rpt, sizeof(int) * (row_block + 1));
    cudaMalloc((void **)&dirreg_cid, sizeof(int) * nnz_irreg);
    cudaMemcpy(dirreg_val, irreg_val, sizeof(double) * nnz_irreg, cudaMemcpyHostToDevice);
    cudaMemcpy(dirreg_rpt, irreg_rpt, sizeof(int) * (row_block + 1), cudaMemcpyHostToDevice);
    ...
    cudaMemcpy(Y_val, dY_val, sizeof(double) * rowA, cudaMemcpyDeviceToHost);
